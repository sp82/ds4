...
SHADERS = [
    ...
    ("attn_output_low_q8_v2", "attention.hlsl", "attn_output_low_q8_v2",
     ["-T", "cs_6_4"]),
    ("attn_output_low_q8_v3", "attention.hlsl", "attn_output_low_q8_v3",
     ["-T", "cs_6_4"]),
...
def main():
    dxc = find_dxc()
    # Unique temp names so parallel invocations (the Makefile can run this
    # recipe from more than one goal with -j) never clobber each other, and
    # the output is swapped in atomically.
    pid = os.getpid()
    tmp_spv = os.path.join(HERE, ".tmp.shader.%d.spv" % pid)
    tmp_out = os.path.join(HERE, ".tmp.shaders.%d.inc" % pid)
